...
仓库的中立默认配置只启用私人待办插件；浏览器自动化、联网检索、资料抓取、
OCR 和 Codex 均默认关闭，启用及修改设置后需要完整重启。
...
`web_crawler` 是持久化的网站资料抓取插件。它按组织保存 HTTP/HTTPS 站点、
广度优先队列、运行、页面版本和结构化记录，遵守 `robots.txt`，支持 sitemap、
HTML/PDF、条件请求、静态优先与 Playwright 动态回退，并可把每个页面更新到
组织知识库。Owner/Admin 在 `/organization/crawler` 管理抓取源，普通成员可
查看和手动发起运行。首版不支持登录、验证码、代理池或反爬绕过，详细规则见
[`docs/web-crawler.md`](docs/web-crawler.md)。
...
当前 SQLite 格式为 v4。受支持的 v1-v3 数据库会在启动时先自动备份，再升级
到当前格式；未知或不兼容版本仍会保持原文件不变并提示管理员处理。
