...
- **工具**：统一查看内置与插件工具的来源、启停、审批策略和执行审计，并管理 Skill 与 MCP；
- **插件中心**：安装、更新、启停和配置可信本地插件；
- **消息渠道**：在 `/channels` 添加、配置和检查 iLink、企业微信与飞书渠道；
- **定时任务**：管理 cron 任务，改动自动重载调度；
- **用户管理**：机器人用户（租户列表/详情/删除）、组织管理（创建、改名、暂停/恢复、成员查看、存量个人空间认领与带备份的删除）、管理员账号（增删改、分配角色、重置密码，新账号初始密码为 `12345`）、角色权限（点击角色弹窗编辑；内置 admin 拥有全部权限且不可修改）。组织管理仅对平台管理员开放，组织成员使用 `/app` 工作台管理所属组织；
- **文档说明**：面板功能与使用指南。
...
├── channels.json    消息渠道实例（微信 iLink、企业微信、飞书）
